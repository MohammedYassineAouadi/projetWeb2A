...
<head>
    <meta charset="UTF-8">
    <title>Connexion - Startup Academy</title>
...
<body>
    ...
    <div class="form-container">
        ...
        <?php if (isset($_GET['success']) && $_GET['success'] === 'passwordreset'): ?>
            <p class="success-global">✅ Votre mot de passe a été réinitialisé avec succès.</p>
        <?php endif; ?>
        <?php if (isset($_GET['error']) && $_GET['error'] === 'google_msg'): ?>
            <?php
            $googleErrorMsg = 'Une erreur est survenue lors de la connexion avec Google.';
            if (isset($_GET['msg'])) {
                switch ($_GET['msg']) {
                    case 'missing_code':
                        $googleErrorMsg = "Erreur Google : code manquant.";
                        break;
                    case 'token_error':
                        $googleErrorMsg = "Erreur Google : impossible d'obtenir le jeton d'accès.";
                        break;
                    case 'userinfo_error':
                        $googleErrorMsg = "Erreur Google : impossible de récupérer les informations utilisateur.";
                        break;
                    case 'user_not_found':
                        $googleErrorMsg = "Aucun compte associé à cet email Google n'a été trouvé.";
                        break;
                }
            }
            ?>
            <p class="error-global" style="color:red; font-weight:bold; text-align:center; margin-bottom:10px;">
                <?= htmlspecialchars($googleErrorMsg) ?>
            </p>
        <?php endif; ?>
        <?php if (isset($_GET['error']) && $_GET['error'] === 'github_msg'): ?>
            <?php
            $githubErrorMsg = 'Une erreur est survenue lors de la connexion avec GitHub.';
            if (isset($_GET['msg'])) {
                switch ($_GET['msg']) {
                    case 'missing_code':
                        $githubErrorMsg = "Erreur GitHub : code manquant.";
                        break;
                    case 'token_error':
                        $githubErrorMsg = "Erreur GitHub : impossible d'obtenir le jeton d'accès.";
                        break;
                    case 'userinfo_error':
                        $githubErrorMsg = "Erreur GitHub : impossible de récupérer les informations utilisateur.";
                        break;
                    case 'user_not_found':
                        $githubErrorMsg = "Aucun compte associé à cet email GitHub n'a été trouvé.";
                        break;
                }
            }
            ?>
            <p class="error-global" style="color:red; font-weight:bold; text-align:center; margin-bottom:10px;">
                <?= htmlspecialchars($githubErrorMsg) ?>
            </p>
        <?php endif; ?>


        <form id="loginForm" action="../Controleur/auth.php?action=login" method="POST">
            ...
            <p class="login-link">Pas encore inscrit ? <a href="inscription.php">Créer un compte</a></p>

        </form>
        <div class="social-login-wrapper">
            <a href="../Controleur/google_login.php" class="social-btn google-btn">
            ...
            <a href="../Controleur/github_login.php" class="social-btn github-btn">
