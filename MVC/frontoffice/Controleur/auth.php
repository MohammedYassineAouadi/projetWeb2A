...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'register' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($_POST['g-recaptcha-response'])) {
            header('Location: ../Vue/inscription.php?error=captcha');
            exit;
        }
        ...
        if ($utilisateurModel->emailExiste($email)) {
            header('Location: ../Vue/inscription.php?error=exists');
            exit;
        }
    ...
    if ($_GET['action'] == 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$utilisateur) {
            header('Location: ../Vue/connexion.php?error=email');
            exit;
        }

        if (!password_verify($mot_de_passe, $utilisateur['mot_de_passe'])) {
            header('Location: ../Vue/connexion.php?error=password');
            exit;
        }
