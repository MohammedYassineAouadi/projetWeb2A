...
if (!isset($_GET['code'])) {
    ...
    $redirect = $isSignup ? '../Vue/inscription.php?error=github_signup_msg&msg=missing_code' : '../Vue/connexion.php?error=github_msg&msg=missing_code';
...
if (!isset($token_data['access_token'])) {
    $isSignup = (isset($_GET['state']) && $_GET['state'] === 'signup');
    $redirect = $isSignup ? '../Vue/inscription.php?error=github_signup_msg&msg=token_error' : '../Vue/connexion.php?error=github_msg&msg=token_error';
    header('Location: ' . $redirect);
    exit;
}
...
if ($email) {
    ...
    if ($isSignup) {
        // SIGNUP FLOW
        if ($utilisateur) {
            header('Location: ../Vue/inscription.php?error=github_signup_msg&msg=exists');
            exit;
        }
        ...
        if ($utilisateur) {
            ...
            header('Location: ../Vue/connexion.php?error=github_msg&msg=user_not_found');
            exit;
        }
    }
} else {
    $isSignup = (isset($_GET['state']) && $_GET['state'] === 'signup');
    $redirect = $isSignup ? '../Vue/inscription.php?error=github_signup_msg&msg=userinfo_error' : '../Vue/connexion.php?error=github_msg&msg=userinfo_error';
    header('Location: ' . $redirect);
    exit;
}
