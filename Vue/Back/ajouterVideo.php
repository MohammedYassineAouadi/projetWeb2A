...
$titre = $description = $url = $duree = $date_ajout = "";
$id_pdf = null;
$errors = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($date_ajout)) {
        $errors['date_ajout'] = "❌ La date d'ajout est obligatoire.";
    } elseif (!DateTime::createFromFormat('Y-m-d', $date_ajout)) {
        $errors['date_ajout'] = "❌ Le format de la date est invalide (attendu : AAAA-MM-JJ).";
    }
    ...
    // Si pas d'erreurs, on ajoute la vidéo
    if (empty($errors)) {
        $video = new Video($titre, $description, $url, $duree, $date_ajout, (int)$id_pdf);
        $videoController = new VideoC();
        $ajout = $videoController->ajouterVideo($video);

        if ($ajout) {
            echo "<p style='color: green;'>✅ Vidéo ajoutée avec succès.</p>";
            $titre = $description = $url = $duree = $date_ajout = "";
            $id_pdf = null;
        } else {
            echo "<p style='color: red;'>❌ Erreur lors de l'ajout de la vidéo.</p>";
        }
    }
}
?>
...
<body>
	...
	<div class="main-container">
		<div class="pd-ltr-20 xs-pd-20-10">
			<div class="min-height-200px">
				...
				<div class="pd-20 card-box mb-30">
					...
					<form method="POST" action="ajouterVideo.php">
        <!-- Titre -->
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="duree" value="<?= htmlspecialchars($duree) ?>">
        ...
        <div class="form-group">
            <label>Date</label>
            <input class="form-control" type="date_ajout" name="date_ajout" value="<?= htmlspecialchars($date_ajout) ?>">
        ...
        <!-- Bouton -->
        <button class="btn btn-primary" type="submit">Ajouter Vidéo</button>
    </form>
